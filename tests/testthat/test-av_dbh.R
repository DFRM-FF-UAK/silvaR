...
test_that("av_dbh returns correct mean dbh when only_measured_h is TRUE", {
  result <- round(av_dbh(plot_id, tree_id, species, age, layer, dbh, height, only_measured_h = TRUE),1)
  expected_result <- c(32.5,32.5,31.0,NA,28.0,NA,12.0,NA,NA,48.0,31.3,31.3,31.3,31.3)
  expect_equal(result, expected_result)
})

test_that("av_dbh returns correct mean dbh when only_measured_h is FALSE", {
  new_dbh <- dbh
  new_dbh[4] <- 14
  result <- av_dbh(plot_id, tree_id, species, age, layer, new_dbh, only_measured_h = FALSE)
  expected_result <- c(32.50,32.50,31.00,14.00,28.00,47.00,12.00,31.50,31.50,48.00,31.75,31.75,31.75,31.75)
  expect_equal(result, expected_result)
})

test_that("av_dbh throws an error when only_measured_h is TRUE but height is missing", {
...
test_that("av_dbh warns for groups with no dbh measurements when only_measured_h is FALSE", {
